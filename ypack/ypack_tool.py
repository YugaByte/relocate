...
class YPackTool():
    ...
    def parse_args(self, args):
        ...
        parser_for_create.add_argument('--src', help='Source directory to package', required=True)
        parser_for_create.add_argument('--dest', help='Directory to create the package in', required=True)
        parser_for_create.set_defaults(func=self.create)
        self.args = parser.parse_args()
        self.src_dir = self.args.src
        self.dest_parent_dir = self.args.dest
        if not os.path.isdir(self.src_dir):
            raise IOError("Source directory '%s' does not exist" % self.src_dir)
        ...
        self.patterns = {
            'install_dir': remove_prefixes_of_others(
                [os.path.abspath(self.src_dir),
                 os.path.realpath(self.src_dir)]
            )
        }

        self.dir_to_search = self.src_dir
        if os.path.islink(self.src_dir):
            link_target_dir = os.path.realpath(os.readlink(self.src_dir))
            self.dir_to_search = link_target_dir
    ...
    def create(self):
        ...
        for pattern_value in sorted(pattern_values):
            ...
            for line in out.decode('utf-8').split('\n'):
                ...
                rel_path = os.path.relpath(file_path, self.dir_to_search)
